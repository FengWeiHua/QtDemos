    ...
    , mPictureSelectionModel(nullptr)
{
    ...
    connect(ui->addPicutresButton, &QPushButton::clicked,
...
{
    QStringList filenams = QFileDialog::getOpenFileNames(this,
                                                         "Add pictures",
                                                         QDir::homePath(),
                                                         "Pciture files (*.jpg *.png)");
    if (filenams.isEmpty()) {
        return;
    }

    QModelIndex lastModelIndex;
    for (auto filename : filenams) {
...
void AlbumWidget::clearUi()
{
    ...
    ui->addPicutresButton->setVisible(false);
}
...
void AlbumWidget::loadAlbum(const QModelIndex &albumIndex)
{
    ...
    ui->addPicutresButton->setVisible(true);
}
